...
//          TextOut(FLeft, FTop, FText);         {contents of Barcode}
          //TextOut(PosX, PosY, FText);         {contents of Barcode}
            TextOut(PosX, PosY, FormattedCode);
...
function TAsBarcode.FormattedCode: string;
var i:integer;
var str:string;
begin
    str :=  ' '+  FText +' ';
    if FFormatSerialText then
    begin
        for  i:=1 to  length(FText)do
        begin
            if (( i>0) and (i mod 4=0))  then
                 str:=str+copy(Ftext,i, 1)+' '
            else
               str:=str+copy(Ftext,i, 1);
        end;
    end;
...
procedure TAsBarcode.SetFormatSerialText(const Value: boolean);
begin
