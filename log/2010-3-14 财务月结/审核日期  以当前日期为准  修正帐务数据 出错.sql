...
if exists(select *from sysobjects where name='VGetDate')
drop view VGetDate
go
create view VGetDate
as
select getdate()  as Fgetdate
go

if exists(select *from sysobjects where name='Fn_IsMonthClose')
drop function Fn_IsMonthClose
go

create  function Fn_IsMonthClose
(@Fdate smalldatetime) returns smalldatetime
as
begin
	
	declare @rltdate smalldatetime
        declare @CurGetDate smalldatetime
        select  @CurGetDate =Fgetdate from VGetDate

	if exists( select * from TMonthEndClose  where isClosed=1 and  year(fMonth)=year(@Fdate) and month(fMonth)=month(@Fdate) ) 
        set   @rltdate =@CurGetDate 

	return  isnull( isnull(@rltdate,@Fdate  ),@CurGetDate)
end
...
go
alter   PROCEDURE [dbo].[sl_retail_chk1]     
                 @RetailId varchar(20),    
                 @HandManId varchar(20)    
AS    
...
--*/    
     begin tran    
       ...
       update sl_retail set IsChk=1,ChkManId=@HandManId,ChkTime=dbo.Fn_IsMonthClose (ChkTime)  ,WhOutCode=@OutCode where Code=@RetailId    
...
go
alter  PROCEDURE [dbo].[wh_out_chk1]   
                 @OutId varchar(20),  
                 @HandManId varchar(20),  
                 @LoginId varchar(20)  
AS  
...
--*/  
     begin tran  
       update wh_out set IsChk=1,ChkManId=@HandManId,ChkTime=dbo.Fn_IsMonthClose (ChkTime)   where Code=@OutId     
     commit  
     if @@error=0  
        return(1)  
     rollback  
...
AS    
 ...
 begin    
     ...
     set @ChkManId=@HandManId    
       ...
       if @Pre='FR'    
         update fn_receiver set IsChk=@IsChk,ChkManId=@ChkManId,ChkTime=dbo.Fn_IsMonthClose (ChkTime)      where Code=@ReceiverId    
       else if @Pre='SI'    
         update sl_invoice set FnIsChk=@IsChk,FnChkManId=@ChkManId,FnChkTime=dbo.Fn_IsMonthClose (FnChkTime)  where Code=@ReceiverId    
        else if @Pre='FP'    
         update fn_payment set IsChk=@IsChk,ChkManId=@ChkManId,ChkTime=dbo.Fn_IsMonthClose (ChkTime)   where Code=@ReceiverId    
       else if @Pre='RT'    
         update sl_retail set FnIsChk=@IsChk,FnChkManId=@ChkManId,FnChkTime= dbo.Fn_IsMonthClose (FnChkTime)    where Code=@ReceiverId    
       else if @Pre='FT'    
       begin    
         ...
         begin    
           ...
           begin    
             ...
             update fn_bnktransfer set IsxChk=@IsChk,xChkManId=@ChkManId,xChkTime= dbo.Fn_IsMonthClose (xChkTime)  where Code=@ReceiverId-- and OutId=@BankId    
         end    
         else begin    
           ...
           begin    
             ...
             update fn_bnktransfer set IsiChk=@IsChk,iChkManId=@ChkManId,iChkTime=dbo.Fn_IsMonthClose (iChkTime)   where Code=@ReceiverId --and InId=@BankId    
         end    
       end    
...
alter PROCEDURE [dbo].[fn_shldinout_chk]   
                 @LoginId varchar(20),  
                 @HandManId varchar(20),  
                 @ShldId varchar(20),  
                 @IsChk bit  
 ...
 begin  
   ...
   begin  
     ...
     return 0  
       ...
       if @Pre='YI'  
         update fn_shldin set IsChk=@IsChk,ChkManId=@ChkManId,ChkTime=dbo.Fn_IsMonthClose (ChkTime) where Code=@ShldId    --IsNull(ChkTime,GetDate())
       else if @Pre='YO'  
         update fn_shldout set IsChk=@IsChk,ChkManId=@ChkManId,ChkTime= dbo.Fn_IsMonthClose (ChkTime) where Code=@ShldId  
       else if @Pre='FC'  
         update fn_clntransfer set IsChk=@IsChk,ChkManId=@ChkManId,ChkTime= dbo.Fn_IsMonthClose (ChkTime)  where Code=@ShldId  
