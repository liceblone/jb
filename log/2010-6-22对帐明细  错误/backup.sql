CREATE   PROCEDURE [fn_clientdl_inout]                                                   
                 @ClientId varchar(20),                                                  
                 @BeginDate datetime,                                                  
                 @EndDate datetime,                                                  
                 @InOut varchar(10),                                                  
...
/*                                                  
union all                                                  
select 0,1,1,0,null,Code,InDate,InDate,Code,(case IsChk when 1 then 'clBlack' else 'clRed' end),case InTypeId when 'I08' then '借入' when 'I09' then '异地调配入库' end,null,WareFund,null,null from wh_in where  ClientId=@ClientId and (InTypeId='I08' or InTypeId=' 
...
I                                      
                  09') and InDate>=@BeginDate and InDate<=@EndDate             
union all                                                  
select 0,0,1,0,null,m.Code,m.InDate,null,null,'clBlack',d.PartNo+space(21-len(d.PartNo))+d.Brand+space(8-len(d.Brand))+cast(cast (d.Qty as int) as varchar(20))+space(8-len(cast(cast (d.Qty as int) as varchar(20))))+cast(cast(d.Price as decimal(15,5)) as  
...
v      
              ...
              archar(40)),null,null,null,null from wh_indl d,wh_in m where d.InId=m.Code and m.ClientId=@ClientId and (m.InTypeId='I08' or m.InTypeId='I09') and m.InDate>=@BeginDate and m.InDate<=@EndDate                                                   
  
    
      
       
union all                                                  
select 0,1,0,1,null,Code,OutDate,OutDate,Code,(case IsChk when 1 then 'clBlack' else 'clRed' end),case OutTypeId when 'X08' then '借出' when 'X09' then '异地调配出库' end,WareFund,null,null,null from wh_out where ClientId=@ClientId and (OutTypeId='X08' or OutTyp 
...
e                                      
                    Id='X09') and OutDate>=@BeginDate and OutDate<=@EndDate                                                  
union all                                                  
select 0,0,0,0,null,m.Code,m.OutDate,null,null,'clBlack',d.PartNo+space(21-len(d.PartNo))+d.Brand+space(8-len(d.Brand))+cast(cast (d.Qty as int) as varchar(20))+space(8-len(cast(cast (d.Qty as int) as varchar(20))))+cast(cast(d.Price as decimal(15,5)) as 
                     ...
                     varchar(40)),null,null,null,null from wh_outdl d,wh_out m where d.OutId=m.Code and m.ClientId=@ClientId and (m.OutTypeId='X08' or m.OutTypeId='X09') and m.OutDate>=@BeginDate and m.OutDate<=@EndDate                                    
...
/*                                                  
union all                                                  
select Code,InDate,InDate,Code,'异地调配入库',null,WareFund,null from wh_in where IsChk=1 and ClientId=@ClientId and InTypeId='I09' and InDate>=@BeginDate and InDate<=@EndDate                                                  
union all                                                  
select m.Code,m.InDate,null,null,d.PartNo+','+d.Brand+','+cast(d.Price as varchar(20))+'x'+cast(d.Qty as varchar(20)),null,null,null from wh_indl d,wh_in m where d.InId=m.Code and m.IsChk=1 and m.ClientId=@ClientId and m.InTypeId='I08' and m.InDate>=@Beg 
...
nDate and m.InDate<=@EndDate                                                  
union all                                                  
select Code,OutDate,OutDate,Code,'异地调配出库',WareFund,null,null from wh_out where IsChk=1 and ClientId=@ClientId and OutTypeId='X09' and OutDate>=@BeginDate and OutDate<=@EndDate                                      
union all                                                  
select m.Code,m.OutDate,null,null,d.PartNo+','+d.Brand+','+cast(d.Price as varchar(20))+'x'+cast(d.Qty as varchar(20)),null,null,null from wh_outdl d,wh_out m where d.OutId=m.Code and m.IsChk=1 and m.ClientId=@ClientId and m.OutTypeId='X08' and m.OutDate 
...
=@BeginDate and m.OutDate<=@EndDate                                                  
*/                                      
union all                                                  
