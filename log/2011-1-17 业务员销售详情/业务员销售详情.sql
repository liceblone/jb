...
set @EndDate=@EndDate+1        
   ...
   union all        
   select ClientId,null,OutDate,0,PayFund from fn_shldout where IsChk=1 and ItemId='0100' and OutDate>@BeginDate and OutDate<@EndDate        
   union all        
...
--   delete from #a where ClientId in (select ClientId from sl_empclient where EmpId<>@EmpId)      select top 1 * From fn_payment  fn_clntransfer  
...
--//        
   ...
   select Code,Name,null as Jan,null as Janr,        
         ...
         null as tFundx,null as times into #b from crm_client where Code in (select ClientId from sl_empclient where EmpId=@EmpId)--(select ClientId from #a group by ClientId)        
   ...
   update #b set Jan=(select sum(Fund) from #a where ClientId=Code and DatePart(month,xDate)=1),        
                 ...
                 tFundr=(select sum(PayFund) from #a where ClientId=Code),        
                 tFundx=(select sum(Fundx) from #c where ClientId=Code),        
   ...
   union all        
   select 0,'clRed',null,'合计',sum(Jan),sum(Feb),sum(Mar),sum(Apr),sum(May),sum(Jun),sum(Jul),sum(Aug),sum(Sep),sum(Oct),sum(Nov),sum(Dece),sum(tFund),sum(Janr),sum(Febr),sum(Marr),sum(Aprr),sum(Mayr),sum(Junr),sum(Julr),sum(Augr),sum(Sepr),sum(Octr),sum( 
  
Novr),sum(Decer),sum(tFundr),sum(tFundx),sum(times) from #b    
