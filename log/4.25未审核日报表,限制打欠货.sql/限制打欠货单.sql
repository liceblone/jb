...
if exists (select * from sl_invoicedl  where InvoiceId=@InvoiceId and IsNull(SndQty,0)>0 and IsNull(SndQty,0)>Isnull((select sum(d.qty) from wh_in m inner join wh_indl d  on m.Code=d.InId and m.IsChk=1 and m.WhId=@WhId and d.WareId=sl_invoicedl.WareId),0)

-Isnull((select sum(d.qty) from wh_out m inner join wh_outdl d on m.Code=d.OutId and m.IsChk=1 and m.WhId=@WhId and d.WareId=sl_invoicedl.WareId),0)+Isnull((select sum(Qty) from wh_regulate where WhId=@WhId and WareId=sl_invoicedl.WareId),0))            
...
begin              
  ...
  return 0              
        ...
        set @msg=''              
        select @msg=partNo+'     '+isnull(brand ,'')+'     '+isnull(pack,'') from sl_invoicedl  where InvoiceId=@InvoiceId and RmnQty>0  
        if @msg<>'' 
	begin
              set @msg='审核失败,原因是: '+@msg+'  库存不足，并且不能打欠货单'
	         Raiserror(@msg,16,1)              
              return 0 
        end   
...
---update wr_ware set price=cost where isnull(price,0)=0  and  isnull(cost,0)<>0 and id in (select wareid         from sl_invoicedl             where InvoiceId=@InvoiceId)         
    
      
 /*      
      
select GatheringEmp ,*From crm_client where code='hz000004'      
