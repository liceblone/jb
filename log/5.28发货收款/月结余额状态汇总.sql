...
exec Pr_MonthEndClose  @abortstr output ,@worningstr output,'0021','2009/5/1',0
...
--if @isClosed =1 then insert TCltMonthBalance else delete TCltMonthBalance 
			...
			select clientid , xfund,  fyear , fmonth  into #notExistAmt  from #K    --exists  this month amt plus  last month amt
                        where clientid   in (select clientid From TCltMonthBalance  where year(dateadd(m,-1,@Month))   =chkyear and chkmonth=month (dateadd(m,-1,@Month)) )
                        union all
                        select clientid,balance,year ( @Month ),month ( @Month)    from TCltMonthBalance
                              where year(dateadd(m,-1,@Month))   =chkyear and chkmonth=month (dateadd(m,-1,@Month)) and clientid   in (select clientid From #K)

			insert into TCltMonthBalance(clientid,balance,chkyear,chkmonth) 
                        select clientid ,sum(xfund) ,  fyear , fmonth   from  #notExistAmt group by clientid ,  fyear , fmonth  



			insert into TCltMonthBalance(clientid,balance,chkyear,chkmonth)                           --not exists  use  last month amt
                        select clientid,balance,year ( @Month ),month ( @Month)    from TCltMonthBalance
