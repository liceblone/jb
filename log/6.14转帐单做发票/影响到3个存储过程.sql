...
where M1.Code=M2.ItemTablebillcode and m2.code=@code and Itemtablename='fn_shldout'               
 ...
 from TinvoiceMsgDL A          
 join #billCode     B on B.itemtablebillcode=A.itemtablebillcode   AND Itemtablename='fn_clntransfer'  and  Qty>0  
 group by B.itemtablebillcode  
   
     --  IF  EXISTS(SELECT *FROM #fnCltTrBillInvFund)  
       BEGIN  
  -- update fn_clntransfer set  invqty=isnull( (select isnull( invQty,0) from #fnCltTrBillInvFund where  itemtablebillcode  = code  ),0)   
         -- where  istax=1     and inid=@clientid  
  
         update fn_clntransfer set  invqty=B.invQty  
         from   fn_clntransfer A  
         join   #fnCltTrBillInvFund  B on b.itemtablebillcode  = A.code   
         where  A.istax=1     and A.inid=@clientid  
       END  
  
--join TinvoiceMsg            M on M.code=B.code and M.clientid=A.inid  
  
--  select inid,* from fn_clntransfer  
--select * from TinvoiceMsg  
  
  
--转 出   
  update fn_clntransfer   set NFpID=@code                           
  from fn_clntransfer M1 ,TinvoiceMsgDL M2          
 where M1.Code=M2.ItemTablebillcode  and m2.code=@code   
   and Itemtablename='fn_clntransfer' and qty<0  
        
  
  
  
  
/*  update fn_clntransfer   set NFpID='ii00004118'                            
   
  from fn_clntransfer M1 ,TinvoiceMsgDL M2          
 where M1.Code=M2.ItemTablebillcode  and m2.code='ii00004118'   
   and Itemtablename='fn_clntransfer' and qty<0  
   update fn_clntransfer   set NFpID='II00004073'                           
  from fn_clntransfer M1 ,TinvoiceMsgDL M2          
 where M1.Code=M2.ItemTablebillcode  and m2.code='II00004073'  
   and Itemtablename='fn_clntransfer'   
  
 select * from  fn_clntransfer  
  
 select * from TinvoiceMsgDL where Itemtablename='fn_clntransfer'   
union all   
 select top 15 * from TinvoiceMsgDL   
      
  select * from  fn_clntransfer where code='FC00002789'  
  
update fn_clntransfer set invqty=null  where code='FC00002789'  
  
update fn_clntransfer set NFPID=null  where code='FC00002789'  
  
  select * from  fn_clntransfer where code='FC00002793'  
  
  select * from  fn_clntransfer where inid='HZ000002'  
union   
 select * from  fn_clntransfer where outid='HZ000002'  
  
update fn_clntransfer set invqty=null  where code='FC00002792'  
  
update fn_clntransfer set NFPID=null  where code='FC00002792'  
  
select sum(isnull(A.qty,0)) as invQty     
from TinvoiceMsgDL A               
join TinvoiceMsg   M on M.code=B.code   
join fn_clntransfer Tr on  Tr.inid=M.clientid   
group by B.itemtablebillcode  
...
1,null,'转入',null,null,PayFund,1 as price,
0,  0,PayFund,PayFund,0,
null, PFpID ,code  ,case when IsChk=1  then 'clblue' else  'clred' end AS FntClr,1 as Input, 

 '转入: '  +convert(varchar(10) ,PayFund ) , Trnsfrdate, billtime,  convert(decimal(15,5), PayFund-isnull(InvQty,0)   ) as RmnInvQty                                
...
0  as billtype,27 as billid,  
1,null,'转出',null,null,-PayFund,1 as price,
0,  0,-PayFund,-PayFund,0,
null, NFpID ,code  ,case when IsChk=1  then 'clblue' else  'clred' end AS FntClr,1 as Input, 

 '转出 :'  +convert(varchar(10) ,-PayFund ) ,  Trnsfrdate, billtime,  -  PayFund  as RmnInvQty                                
