alter  PROCEDURE dbo.fn_clntrnsfr_bfrpst    
...
if @EmpId<>@BillManId    
begin    
  set @AbortStr='对不起,您不是本转帐单的制单人,您不能修改本单据.'    
  return 0    
end    
    
if not @PayFund>0     
begin    
  set @AbortStr='对不起,转帐金额必须大于零,保存中止！'    
  return 0    
end    
if exists(select top 1 * from fn_clntransfer where ((NFPID is not null )or (InvQty>0))  and code =@code)    --7.25   已开票不能修改
begin  
  set @AbortStr='已经开票不能修改'    
  return 0   
end  
...
select   
1 as pk, 'fn_clntransfer' as TableName,  '转入 '+convert(varchar(10), PayFund )  as BillNote,code,    --7.25 转帐单  未审就可以开票and ischk=1
...
null, PFpID ,code  ,case when IsChk=1  then 'clblue' else  'clred' end AS FntClr,1 as Input,   
 ...
 from fn_clntransfer where InId =@ClientID     and IsTax =1       and PayFund<>isnull(InvQty,0)  
union all                                    
  
select   
1 as pk, 'fn_clntransfer' as TableName,  '转出 '+convert(varchar(10), PayFund )  as BillNote,code,  --7.25 转帐单  未审就可以开票and ischk=1
0  as billtype,27 as billid,    
...
null, NFpID ,code  ,case when IsChk=1  then 'clblue' else  'clred' end AS FntClr,1 as Input,   
 ...
 from fn_clntransfer  where OutId =@ClientID and IsTax =1  and     NFpID is null                    
