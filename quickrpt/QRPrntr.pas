...
const
   cQRName = 'QuickReport 4.05'; { This string should not be resourced }
   cQRVersion = 40;
...
{$define  OLDAPPLYSETTINGS}
{$ifndef OLDAPPLYSETTINGS}
 ...
 if hDevMode <> 0 then
  begin
    ...
    if hNewDevMode <> 0 then
      try
        ...
        begin
            ...
            rc := DocumentProperties( THandle(0), hPrinter, Device, NewDevMode^, NewDevMode^,
                                       DM_OUT_BUFFER or DM_IN_BUFFER);
            // Duplex
            if ( NewDevMode^.dmFields and dm_Duplex) = dm_Duplex then
            begin
              if ExtendedDuplex <> 0 then
                 NewDevMode^.dmDuplex := ExtendedDuplex
              else
              begin
                 if Duplex then
            ...
            // Orientation 1, 2, 3 are the values in Windows.pas
            if Orientation=poPortrait then
               ...
               NewDevMode^.dmOrientation := dmorient_landscape
                ...
                NewDevMode^.dmOrientation := 3;
            // bin code
            if ( NewDevMode^.dmFields and dm_DefaultSource) = dm_DefaultSource then
            begin
               if  UseCustomBinCode then
                     NewDevMode^.dmDefaultSource := CustomBinCode
               else
                     NewDevMode^.dmDefaultSource := cQRBinTranslate[OutputBin];
            end;
            // paper size
            if ( NewDevMode^.dmFields and dm_Paperlength) = dm_Paperlength then
                          NewDevMode^.dmPaperLength := PaperLength;
            if ( NewDevMode^.dmFields and dm_Paperwidth) = dm_Paperwidth then
                          NewDevMode^.dmPaperWidth := PaperWidth;
            if (( NewDevMode^.dmFields and dm_papersize) = dm_papersize) and (PaperSize <> Default) then
            begin
               if not UseCustomPaperCode then
                    NewDevMode^.dmPaperSize := cQRPaperTranslate[PaperSize]
               else
                    NewDevMode^.dmPaperSize := CustomPaperCode;
            end;
            if ( NewDevMode^.dmFields and dm_copies) = dm_copies then
                     NewDevMode^.dmCopies := Copies;
            if ( NewDevMode^.dmFields and dm_printquality) = dm_printquality then
                     NewDevMode^.dmPrintQuality := PrintQuality;
            if ( NewDevMode^.dmFields and dm_collate) = dm_collate then
                     NewDevMode^.dmCollate := Collate;
            if ( NewDevMode^.dmFields and dm_color) = dm_color then
                    NewDevMode^.dmColor := ColorOption;
    ...
    begin
        ...
        Cancel := false;
        if assigned(OnApplyPrinterSettings) and (DevMode <> nil) then
