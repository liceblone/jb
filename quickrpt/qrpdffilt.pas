...
begin
  ...
  for K := 0 to FPDF.pagegraphicsfiles.Count-1 do TStringList(FPDF.pagegraphicsfiles.Objects[K]).Free;
  FPDF.pagegraphicsfiles.Free;
  for K := 0 to FPDF.pagetextfiles.Count-1 do TStringList(FPDF.pagetextfiles.Objects[K]).Free;
...
{var
  ...
  TM: Extended;}
begin
  //  if FPDFConcatenating and (FReportnum>0) then Exit;
  ...
  //  FPageWidth:=0.1*PaperWidth*PointsPerMM;
  //  FPageHeight:=0.1*PaperHeight*PointsPerMM;
  FExtTopMargin:=TM;
  FAdjustTM:=FTopMargin;
  FAdjustLM:=FLeftMargin;}
  //  FReportnum:=1;
...
var
  ...
  begin
    ...
    try
      // Root catalog
      WriteStr('%PDF-1.3'+//
      '%'+Chr(197)+Chr(173)+Chr(111)+Chr(129));
      ...
      for J := 0 to FPDF.pagetextfiles.Count-1 do begin
        ...
        if FTextFirst then begin
          WriteStr('BT');
          WriteStrNoCRLF(TStringList(FPDF.pagetextfiles.Objects[J]).GetText);
          WriteStrNoCRLF(TStringList(FPDF.pagegraphicsfiles.Objects[J]).GetText);
        end else begin
          // graphics
          WriteStrNoCRLF(TStringList(FPDF.pagegraphicsfiles.Objects[J]).GetText);
          WriteStr('BT');
...
GetDIBits( (GControl as TQRImage).canvas.handle,
  ...
  begin
    ...
    if CompressionOn then begin
      // RunLengthEncodeFilter(Str1, Str2);
      //     stringtofile('test.dat',       StreamToString(str1));
      LZWEncode(Str1, Str2);
      //      AsciiHexEncodeFilter(Str2, Str1);
      Ascii85EncodeFilter(Str2, Str1);
      PDFItem.ImageData:=StreamToString(Str1);
      //      PDFItem.ImageFilter:='/ASCII85Decode /RunLengthDecode';
      //      PDFItem.ImageFilter:='/ASCIIHexDecode /RunLengthDecode';
