object analyserFrm: TanalyserFrm
  Left = 303
  Top = 324
  Width = 757
  Height = 441
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = #23435#20307
  Font.Style = []
  FormStyle = fsMDIChild
  KeyPreview = True
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  WindowState = wsMaximized
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object CoolBar1: TCoolBar
    Left = 0
    Top = 0
    Width = 749
    Height = 40
    AutoSize = True
    Bands = <
      item
        Control = ToolBar1
        ImageIndex = -1
        MinHeight = 36
        Width = 745
      end>
    Bitmap.Data = {
      
      FFFFFFFFFFFEFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    object ToolBar1: TToolBar
      Left = 9
      Top = 0
      Width = 732
      Height = 36
      AutoSize = True
      ButtonHeight = 36
      ButtonWidth = 84
      Caption = 'ToolBar1'
      Color = clBtnFace
      EdgeBorders = []
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = #23435#20307
      Font.Style = []
      Images = dmFrm.ImageList1
      ParentColor = False
      ParentFont = False
      ShowCaptions = True
      TabOrder = 0
      Transparent = True
      object ToolButton1: TToolButton
        Left = 0
        Top = 0
        Caption = 'ToolButton1'
        ImageIndex = 0
      end
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 177
    Width = 749
    Height = 209
    Align = alClient
    Color = clHighlightText
    Ctl3D = False
    DataSource = dlDataSource1
    ImeName = #20013#25991' ('#31616#20307') - '#24494#36719#25340#38899
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
    ParentCtl3D = False
    ParentShowHint = False
    PopupMenu = dmFrm.DbGridPopupMenu1
    ShowHint = True
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = #23435#20307
    TitleFont.Style = []
    OnCellClick = DBGrid1CellClick
    OnDrawColumnCell = DBGrid1DrawColumnCell
  end
  object BtmPanel1: TPanel
    Left = 0
    Top = 386
    Width = 749
    Height = 28
    Align = alBottom
    BevelOuter = bvNone
    Color = clWhite
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 2
    object Label1: TLabel
      Left = 0
      Top = 0
      Width = 749
      Height = 28
      Align = alClient
      Alignment = taRightJustify
      Color = clCream
      ParentColor = False
      Layout = tlCenter
    end
  end
  object TopPanel1: TPanel
    Left = 0
    Top = 40
    Width = 749
    Height = 137
    Align = alTop
    BevelOuter = bvLowered
    Color = clCream
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 3
    OnDblClick = TopPanel1DblClick
    DesignSize = (
      749
      137)
    object statLabel1: TLabel
      Left = 626
      Top = 113
      Width = 85
      Height = 13
      Alignment = taRightJustify
      Anchors = [akRight, akBottom]
      AutoSize = False
      Caption = '0/0'
      Transparent = True
    end
    object OpnDlDsBtn1: TSpeedButton
      Left = 615
      Top = 12
      Width = 59
      Height = 22
      Flat = True
      Glyph.Data = {
        
        3333333333333333333333666600}
      OnClick = OpnDlDsBtn1Click
      OnMouseDown = OpnDlDsBtn1MouseDown
    end
  end
  object dlAdoDataSet1: TADODataSet
    Connection = dmFrm.ADOConnection1
    LockType = ltReadOnly
    AfterClose = dlAdoDataSet1AfterClose
    AfterScroll = dlAdoDataSet1AfterScroll
    CommandTimeout = 120
    Parameters = <>
    Left = 192
    Top = 144
  end
  object dlDataSource1: TDataSource
    DataSet = dlAdoDataSet1
    Left = 248
    Top = 144
  end
  object ActionList1: TActionList
    Images = dmFrm.ImageList1
    Left = 480
    Top = 144
    object printAction1: TAction
      Caption = #25171#21360
      ImageIndex = 13
      OnExecute = printAction1Execute
    end
    object refreshAction2: TAction
      Caption = #21047#26032
      ImageIndex = 16
      OnExecute = refreshAction2Execute
    end
    object SortAction3: TAction
      Caption = #25490#24207
      ImageIndex = 15
      OnExecute = SortAction3Execute
    end
    object exportAction4: TAction
      Caption = #23548#20986
      ImageIndex = 17
      OnExecute = exportAction4Execute
    end
    object importAction5: TAction
      Caption = #23548#20837
      ImageIndex = 21
      OnExecute = importAction1Execute
    end
    object closeAction6: TAction
      Caption = #36864#20986
      ImageIndex = 8
      OnExecute = closeAction1Execute
    end
    object subAction7: TAction
      Caption = #20998#27573
      ImageIndex = 14
    end
    object finderAction8: TAction
      Caption = #26597#35810
      ImageIndex = 3
      OnExecute = finderAction1Execute
    end
    object dsfinderAction9: TAction
      Caption = #26597#25214
      ImageIndex = 31
      OnExecute = dsfinderAction1Execute
    end
    object WarePropAction10: TAction
      Caption = #23646#24615
      ImageIndex = 28
      OnExecute = WarePropAction1Execute
    end
    object DeleteAction11: TAction
      Caption = #21024#38500
      ImageIndex = 2
      OnExecute = DeleteAction1Execute
    end
    object PickWareAction12: TAction
      Caption = #23548#20837
      ImageIndex = 14
    end
    object TabEditorAction13: TAction
      Caption = #32534#36753
      ImageIndex = 21
      OnExecute = TabEditorAction1Execute
    end
    object NewTabEditorAction14: TAction
      Caption = #26032#22686
      ImageIndex = 33
      OnExecute = NewTabEditorAction1Execute
    end
    object NewBillAction15: TAction
      Category = 'bill'
      Caption = #26032#22686
      ImageIndex = 7
      OnExecute = NewBillAction1Execute
    end
    object EditBillAction16: TAction
      Category = 'bill'
      Caption = #20462#25913
      ImageIndex = 21
      OnExecute = EditBillAction1Execute
    end
    object DeleteBillAction17: TAction
      Category = 'bill'
      Caption = #21024#38500
      ImageIndex = 2
    end
    object ChkBillAction18: TAction
      Category = 'bill'
      Caption = #23457#26680
      ImageIndex = 10
      OnExecute = ChkBillAction1Execute
    end
    object DeposeBillAction19: TAction
      Category = 'bill'
      Caption = #20316#24223
      ImageIndex = 11
    end
    object CloseBillAction20: TAction
      Category = 'bill'
      Caption = #20851#38381
      ImageIndex = 22
      OnExecute = CloseBillAction1Execute
    end
    object VldBillAction21: TAction
      Category = 'bill'
      Caption = #30830#35748
      ImageIndex = 12
      OnExecute = VldBillAction1Execute
    end
    object EditorAction22: TAction
      Caption = #32534#36753
      ImageIndex = 21
      OnExecute = EditorAction1Execute
    end
    object DetailAction23: TAction
      Category = 'bill'
      Caption = #26126#32454
      ImageIndex = 32
      OnExecute = DetailAction1Execute
    end
    object dlBillAction24: TAction
      Category = 'bill'
      Caption = #20973#35777
      ImageIndex = 1
      OnExecute = dlBillAction1Execute
    end
    object CalcIncreaseAction25: TAction
      Caption = 'CalcIncreaseAction1'
    end
    object dlFilterAction26: TAction
      Caption = #36807#28388
      ImageIndex = 19
      OnExecute = dlFilterAction1Execute
    end
    object InitdsFinderAction27: TAction
      Caption = 'InitdsFinderAction1'
    end
    object Print2Action28: TAction
      Caption = #25171#21360
      ImageIndex = 13
      OnExecute = Print2Action1Execute
    end
    object ph1Action29: TAction
      Caption = 'ph1Action1'
      OnExecute = ph1Action1Execute
    end
    object ph3Action30: TAction
      Caption = 'ph3Action1'
    end
    object sl1Action31: TAction
      Caption = 'sl1Action1'
      OnExecute = sl1Action1Execute
    end
    object sl3Action32: TAction
      Caption = 'sl3Action1'
    end
    object whInvAction33: TAction
      Caption = #26356#26032
      ImageIndex = 10
      OnExecute = whInvAction1Execute
    end
    object WhInvbAction34: TAction
      Caption = #21021#22987
      ImageIndex = 26
      OnExecute = WhInvbAction1Execute
    end
    object phArriveXAction35: TAction
      Caption = #21478#21040
      ImageIndex = 1
      OnExecute = phArriveXAction1Execute
    end
    object phOrdAriXAction36: TAction
      Caption = #35814#24773
      ImageIndex = 28
      OnExecute = phOrdAriXAction1Execute
    end
    object mtBillAction37: TAction
      Caption = #21333#25454
      ImageIndex = 32
      OnExecute = mtBillAction1Execute
    end
    object mtFirstAction38: TAction
      Caption = #39318#26465
      ImageIndex = 25
      OnExecute = mtFirstAction1Execute
    end
    object mtPriorAction39: TAction
      Caption = #19978#26465
      ImageIndex = 22
      OnExecute = mtPriorAction1Execute
    end
    object mtNextAction40: TAction
      Caption = #19979#26465
      ImageIndex = 24
      OnExecute = mtNextAction1Execute
    end
    object mtLastAction41: TAction
      Caption = #23614#26465
      ImageIndex = 23
      OnExecute = mtLastAction1Execute
    end
    object dlLocateAction42: TAction
      Caption = #23450#20301
      ImageIndex = 12
      OnExecute = dlLocateAction1Execute
    end
    object mtFilterAction43: TAction
      Caption = #36807#28388
      ImageIndex = 19
      OnExecute = mtFilterAction1Execute
    end
    object mtLocateAction44: TAction
      Caption = #23450#20301
      ImageIndex = 12
      OnExecute = mtLocateAction1Execute
    end
    object phOrdCloseAction45: TAction
      Caption = #20851#38381
      ImageIndex = 10
      OnExecute = phOrdCloseAction1Execute
    end
    object phOrdAriBilAction46: TAction
      Caption = #35814#24773
      Hint = #26597#38405#35813#20379#24212#21830#30340#25152#26377#26410#20851#38381#30340#35746#21333#21040#36135#24773#20917'...'
      ImageIndex = 1
      OnExecute = phOrdAriBilAction1Execute
    end
    object fnCaAction47: TAction
      Caption = #23545#24080
      ImageIndex = 1
      OnExecute = fnCaAction1Execute
    end
    object slWhOutAction48: TAction
      Caption = #23457#26680
      ImageIndex = 10
      OnExecute = slWhOutAction1Execute
    end
    object slHisAction49: TAction
      Caption = #21382#21490
      Hint = #26597#30475#21382#21490#34917#36135#21333'...'
      ImageIndex = 1
      OnExecute = slHisAction1Execute
    end
    object fnBankInoutAction50: TAction
      Caption = #26126#32454
      ImageIndex = 1
      OnExecute = fnBankInoutAction1Execute
    end
    object fncadlAction51: TAction
      Caption = #26126#32454
      ImageIndex = 32
      OnExecute = fncadlAction1Execute
    end
    object whWareInoutAction52: TAction
      Caption = #26126#32454
      ImageIndex = 1
      OnExecute = whWareInoutAction1Execute
    end
    object OpenBillAction53: TAction
      Caption = #25171#24320
      ImageIndex = 0
      OnExecute = OpenBillAction1Execute
    end
    object mtProcAction54: TAction
      Caption = #24323#21333
      ImageIndex = 2
      OnExecute = mtProcAction1Execute
    end
    object slOutStpAction55: TAction
      Caption = #21462#28040
      ImageIndex = 4
      OnExecute = slOutStpAction1Execute
    end
    object fnDyRprtDtlAction56: TAction
      Caption = #35814#24773
      ImageIndex = 1
      OnExecute = fnDyRprtDtlAction1Execute
    end
    object fnAlrdyBlAction57: TAction
      Caption = #20973#25454
      ImageIndex = 1
      OnExecute = fnAlrdyBlAction1Execute
    end
    object fnChkAction58: TAction
      Caption = #23457#26680
      ImageIndex = 10
      OnExecute = fnChkAction1Execute
    end
    object fnDyRprtDtl2Action59: TAction
      Caption = #35814#32454
      ImageIndex = 1
      OnExecute = fnDyRprtDtl2Action1Execute
    end
    object IsInvoiceAction60: TAction
      Caption = #24320#31080
      ImageIndex = 10
      OnExecute = IsInvoiceAction60Execute
    end
    object fnItmInoutAction61: TAction
      Caption = 'fnItmInoutAction61'
      OnExecute = fnItmInoutAction61Execute
    end
    object fnShldOutIOAction62: TAction
      Caption = 'fnShldOutIOAction62'
      OnExecute = fnShldOutIOAction62Execute
    end
    object wrprice63: TAction
      Caption = #36827#20215
      ImageIndex = 12
      OnExecute = wrprice63Execute
    end
    object IsOutBillAction64: TAction
      Caption = #20986#21333
      ImageIndex = 33
      OnExecute = IsOutBillAction64Execute
    end
    object slInvoiceRecHintAction65: TAction
      Caption = #24310#26399
      ImageIndex = 21
      OnExecute = slInvoiceRecHintAction65Execute
    end
    object slInvoiceRecedAction1: TAction
      Caption = #25910#21040
      ImageIndex = 10
      OnExecute = slInvoiceRecedAction1Execute
    end
    object slEmpSaleDlAction67: TAction
      Caption = #35814#24773
      OnExecute = slEmpSaleDlAction67Execute
    end
    object slClientKp2Action68: TAction
      Caption = #21457#31080
      ImageIndex = 1
      OnExecute = slClientKp2Action68Execute
    end
    object slClientProfitAction69: TAction
      Caption = 'slClientProfitAction69'
      OnExecute = slClientProfitAction69Execute
    end
    object ywyDetailNewAction70: TAction
      Caption = #26032#22686
      ImageIndex = 21
      OnExecute = ywyDetailNewAction70Execute
    end
    object ywyDetailEdtAction71: TAction
      Caption = #32534#36753
      ImageIndex = 21
      OnExecute = ywyDetailEdtAction71Execute
    end
    object ywyLinkmansAction72: TAction
      Caption = #32852#31995#20154
      ImageIndex = 27
      OnExecute = ywyLinkmansAction72Execute
    end
    object ywyActivitysAction73: TAction
      Caption = #27963#21160
      ImageIndex = 33
      OnExecute = ywyActivitysAction73Execute
    end
    object ywyLinkmanNewAction74: TAction
      Caption = #22686#21152
      ImageIndex = 7
    end
    object ywyActivityNewAction75: TAction
      Caption = #22686#21152
      ImageIndex = 7
    end
    object kcYdstockAction76: TAction
      Caption = #24322#22320
      ImageIndex = 18
      OnExecute = kcYdstockAction76Execute
    end
    object phQuotePriceAction77: TAction
      Caption = #20379#25253
      Hint = #21442#38405#20379#36135#25253#20215
      ImageIndex = 35
      OnExecute = phQuotePriceAction77Execute
    end
    object phOrderReferAction78: TAction
      Caption = #35746#21442
      Hint = #22312#35746#21442#32771
      ImageIndex = 1
      OnExecute = phOrderReferAction78Execute
    end
    object phArriveReferAction79: TAction
      Caption = #36827#21442
      ImageIndex = 12
      OnExecute = phArriveReferAction79Execute
    end
    object ywyDemandAction80: TAction
      Caption = #38656#27714
      ImageIndex = 6
      OnExecute = ywyDemandAction80Execute
    end
    object IsInvEditAction81: TAction
      Caption = #32534#36753
      ImageIndex = 21
      OnExecute = IsInvEditAction81Execute
    end
    object ActionStkinAndStkout82: TAction
      Caption = #26576#31181#22120#20214#30340#20986#20837#24211#27719#20817
      ImageIndex = 32
      OnExecute = ActionStkinAndStkout82Execute
    end
    object actInputInvoice83: TAction
      Caption = #21457#31080#24405#20837
      ImageIndex = 29
      OnExecute = actInputInvoice83Execute
    end
    object actPackPic84: TAction
      Caption = #23553#35013#22270#29255' '
      ImageIndex = 7
      OnExecute = actPackPic84Execute
    end
    object actBrandPic85: TAction
      Caption = #21697#29260
      ImageIndex = 29
      OnExecute = actBrandPic85Execute
    end
    object actPriceOutRfs86: TAction
      Caption = #21806#21442
      ImageIndex = 1
      OnExecute = actPriceOutRfs86Execute
    end
    object actNeedInvoiceDL: TAction
      Caption = #26126#32454
      ImageIndex = 1
      OnExecute = actNeedInvoiceDLExecute
    end
    object actHaveInvoiceDL: TAction
      Caption = #24050#24320#26126#32454#65288#20572#29992#65289
      ImageIndex = 28
      OnExecute = actHaveInvoiceDLExecute
    end
    object actInvoice: TAction
      Caption = #24320#31080
      ImageIndex = 26
      OnExecute = actInvoiceExecute
    end
    object actShowPInvoice: TAction
      Caption = #34987#20914#28040#21457#31080
      ImageIndex = 4
      OnExecute = actShowPInvoiceExecute
    end
    object actChkORUnChk: TAction
      Caption = #23457#26680'/'#24323#23457
      ImageIndex = 10
      OnExecute = actChkORUnChkExecute
    end
    object actShowNInvoice: TAction
      Caption = #26174#31034#36127#21457#31080
      ImageIndex = 33
      OnExecute = actShowNInvoiceExecute
    end
    object actEditPriceChange: TAction
      Caption = #32534#36753
      ImageIndex = 18
      OnExecute = actEditPriceChangeExecute
    end
    object actShowPriceChange: TAction
      Caption = #20215#26684#21464#21270
      ImageIndex = 18
      OnExecute = actShowPriceChangeExecute
    end
    object InvoiceItem: TAction
      Caption = #26032#21457#31080
      ImageIndex = 28
      OnExecute = InvoiceItemExecute
    end
    object ActUpdateSalePrice: TAction
      Caption = #21806#20215
      ImageIndex = 17
      OnExecute = ActUpdateSalePriceExecute
    end
    object ActInvoiceBillItem: TAction
      Caption = #21333#25454#26126#32454
      ImageIndex = 28
      OnExecute = ActInvoiceBillItemExecute
    end
    object ActCreatePhPlan: TAction
      Caption = #29983#25104#37319#36141#35745#21010
      ImageIndex = 7
      OnExecute = ActCreatePhPlanExecute
    end
    object ActQueryPhPlan: TAction
      Caption = #26597#35810#37319#36141#35745#21010
      ImageIndex = 28
    end
    object ActAdjustStock100: TAction
      Caption = #26356#25913#38169#35823#36164#26009
      ImageIndex = 17
      OnExecute = ActAdjustStock100Execute
    end
    object ActSP_sl_daysaleDL: TAction
      Caption = 'ActSP_sl_daysaleDL101'
      OnExecute = ActSP_sl_daysaleDLExecute
    end
    object ActInvoiceWare: TAction
      Caption = #32534#36753
      ImageIndex = 21
      OnExecute = ActInvoiceWareExecute
    end
    object ActOriginalBill: TAction
      Caption = 'ActOriginalBill'
      Hint = #22686#20943#26597#35810#19987#29992
      ImageIndex = 32
      OnExecute = ActOriginalBillExecute
    end
    object ActIsDuplicateClt: TAction
      Caption = #37325#22797
      ImageIndex = 2
      OnExecute = ActIsDuplicateCltExecute
    end
    object ActNotDuplicateClt: TAction
      Caption = #27809#37325#22797
      ImageIndex = 10
      OnExecute = ActNotDuplicateCltExecute
    end
    object ActSlInvoiceRecDl: TAction
      Caption = 'ActSlInvoiceRecDl'
      ImageIndex = 28
      OnExecute = ActSlInvoiceRecDlExecute
    end
    object ActOldSlRecDL: TAction
      Caption = #32769#24080#21457#36135#25910#27454
      OnExecute = ActOldSlRecDLExecute
    end
    object ActTranBill: TAction
      Caption = #32534#36753
      ImageIndex = 32
      OnExecute = ActTranBillExecute
    end
    object ActOutBillLog: TAction
      Caption = #20986#21333#26085#24535
      ImageIndex = 28
      OnExecute = ActOutBillLogExecute
    end
    object ActGatheringChk110: TAction
      Caption = #25910#27454#30830#35748
      ImageIndex = 10
      OnExecute = ActGatheringChk110Execute
    end
    object ActMonthEndClose: TAction
      Caption = #26376#32467
      ImageIndex = 10
      OnExecute = ActMonthEndCloseExecute
    end
    object ActMonthEndHis: TAction
      Caption = #26376#32467#35760#24405
      ImageIndex = 32
      OnExecute = ActMonthEndHisExecute
    end
    object ActCustArrivalDL: TAction
      Caption = #29790#33832#21457#31080#26126#32454
      ImageIndex = 32
      OnExecute = ActCustArrivalDLExecute
    end
    object ActPuOrd: TAction
      Caption = #37319#36141#35746#21333
      ImageIndex = 1
      OnExecute = ActPuOrdExecute
    end
    object ActCustBill: TAction
      Caption = #29790#33832#21457#31080
      ImageIndex = 1
      OnExecute = ActCustBillExecute
    end
    object ActPOrdExeProgress: TAction
      Caption = #35746#21333#25191#34892#26126#32454
      ImageIndex = 32
      OnExecute = ActPOrdExeProgressExecute
    end
    object ActPayBill: TAction
      Caption = #20184#27454
      ImageIndex = 1
      OnExecute = ActPayBillExecute
    end
    object ActXlkInvoice: TAction
      Caption = #20449#21033#24247#21457#31080
      ImageIndex = 1
      OnExecute = ActXlkInvoiceExecute
    end
    object ActSortAnalyser: TAction
      Caption = #25490#24207
      ImageIndex = 15
      OnExecute = ActSortAnalyserExecute
    end
    object ActExport120: TAction
      Caption = #23548#20986
      ImageIndex = 40
      OnExecute = ActExport120Execute
    end
    object ActReloadGridFormat: TAction
      Caption = #21152#36733#21015#35774#32622
      ImageIndex = 26
      OnExecute = ActReloadGridFormatExecute
    end
  end
  object mtADODataSet1: TADODataSet
    Connection = dmFrm.ADOConnection1
    CommandTimeout = 120
    Parameters = <>
    Left = 280
    Top = 56
    object mtADODataSet1IntegerField111: TIntegerField
      FieldKind = fkCalculated
      FieldName = '111'
      Calculated = True
    end
  end
  object mtDataSource1: TDataSource
    DataSet = mtADODataSet1
    Left = 344
    Top = 56
  end
end
