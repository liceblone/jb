...
uses
   Windows, Classes, SysUtils,Dialogs,EncdDecd;
type
...
begin
 ...
 Result :=EncdDecd.EncodeString(des(Key, Str, 1, 1, str1));
end;
...
var i:integer;
 var vistr:string;
begin
str:= EncdDecd.DecodeString  (str);

   for i := 0 to 7 do
    begin
     vistr :=vistr + Chr(vi[i]);
    end;
   Result := trim(des(Key,  (Str), 0, 1, vistr));
end;
