...
type
  TWrTransformBillFrm = class(TFrmModel)
...
var
  WrTransformBillFrm: TWrTransformBillFrm;

implementation
uses editor,RepBill,datamodule,TabGrid,Mailer,Downer, desktop;
{$R *.DFM}

procedure TWrTransformBillFrm.InitFrm(FrmId:String);
 var i:integer;
begin
...
procedure TWrTransformBillFrm.OpenBill(fCode:String);
begin
...
procedure Twrtransformbillfrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure Twrtransformbillfrm.DlDataSource1StateChange(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.mtDataSource1StateChange(Sender: TObject);
begin
  if (MtDataSource1.State=dsEdit) or (MtDataSource1.State=dsInsert) then
     EditPostAfter(false);
end;

procedure Twrtransformbillfrm.ScrollBox1CanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
  Resize:=NewHeight>50;
end;

procedure Twrtransformbillfrm.SetBtmBoxHeight(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.dlDataSet1CalcFields(DataSet: TDataSet);
begin
...
procedure Twrtransformbillfrm.CloseBill;
begin
   mtDataSet1.Close;
   dlDataSet1.Close;
   OpenCloseAfter(false);
...
procedure Twrtransformbillfrm.SetRitBtn;
begin
...
procedure Twrtransformbillfrm.OpenCloseAfter(IsOpened:Boolean);
begin
...
procedure Twrtransformbillfrm.EditPostAfter(IsEnabled:Boolean);
begin
    if IsEnabled then
       SetRitBtn
    else begin
...
procedure Twrtransformbillfrm.CpyBtnClick(Sender: TObject);
...
procedure Twrtransformbillfrm.SetBillStyle(fReadOnly:Boolean);
begin
...
procedure Twrtransformbillfrm.SetCtrlStyle(fEnabled:Boolean);
 var bkColor:TColor;
begin
...
procedure Twrtransformbillfrm.SetCheckStyle(IsChecked:Boolean);
begin
...
procedure Twrtransformbillfrm.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
{A.浏览状态下
  1.Ctrl+p: 打印单据
...
procedure Twrtransformbillfrm.PrintAction1Execute(Sender: TObject);
var
  bk:TBookmark;
begin
...
procedure Twrtransformbillfrm.OpenAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.NewAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.RemoveAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.CancelAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.SaveAction1Execute(Sender: TObject);
var
  //istmp:Boolean;
  s,ss,f:widestring;
var i:integer;
...
procedure Twrtransformbillfrm.CheckAction1Execute(Sender: TObject);
{
var
  i:integer;
  s:string;
...
begin
  ...
  begin
    ...
    try
      ...
      FhlUser.SetDataSet(dmFrm.FreeDataSet1,'657',varArrayof([fbilldict.billcode,LoginInfo.EmpId]),True);
...
procedure Twrtransformbillfrm.LinkAction1Execute(Sender: TObject);
  var i:integer;
begin
...
procedure Twrtransformbillfrm.ImportAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.AppendAction1Execute(Sender: TObject);
begin
if    not (dlDataSet1.LockType = ltReadOnly) then
 dlDataSet1.Append;
end;

procedure Twrtransformbillfrm.DeleteAction1Execute(Sender: TObject);
begin
if    not (dlDataSet1.LockType = ltReadOnly) then
begin
         if    not dlDataSet1.IsEmpty then
...
procedure Twrtransformbillfrm.RefreshAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.CloseAction1Execute(Sender: TObject);
begin
 Close;
end;

procedure Twrtransformbillfrm.MailAction1Execute(Sender: TObject);
var
  t:TStringList;
  s,ss:string;
  i:integer;
...
procedure Twrtransformbillfrm.FaxAction1Execute(Sender: TObject);
begin
showmessage('传真功能暂未开发!');
end;

procedure Twrtransformbillfrm.DownAction1Execute(Sender: TObject);
begin
...
procedure Twrtransformbillfrm.FormCreate(Sender: TObject);
begin
  DownAction1.Visible:=Not LoginInfo.IsLocalServer;
  
end;

procedure Twrtransformbillfrm.ScrollBox1DblClick(Sender: TObject);
...
procedure Twrtransformbillfrm.DBGrid1DblClick(Sender: TObject);
var CrtCom:TfrmCreateComponent    ;
begin
// modeltpe:=Bill;
  if (LoginInfo.EmpId ='000') and (LoginInfo.LoginId ='chy') then
...
procedure Twrtransformbillfrm.ScrollBox2DblClick(Sender: TObject);
var CrtCom:TfrmCreateComponent    ;
begin
// modeltpe:=Bill;
  if (LoginInfo.EmpId ='000') and (LoginInfo.LoginId ='chy') then
...
procedure Twrtransformbillfrm.dlDataSet1AfterPost(DataSet: TDataSet);
begin
...
procedure Twrtransformbillfrm.actAftSaveBillExecute(Sender: TObject);
...
procedure Twrtransformbillfrm.ActFormalInvoiceExecute(Sender: TObject);
var    fFrmId,fcode,PreCode:string;
var sql:string;
var CpySourceFlds,CpyDestFlds:string;
var FormalInvoice:Twrtransformbillfrm;
begin
  ...
  //  0301052101
    ...
    PreCode:=self.mtDataSet1.FieldByName(self.fbilldict.mkeyfld ).AsString ;
     ...
     try
      ...
      begin
        FormalInvoice:=Twrtransformbillfrm.Create(self);
        ...
        begin
          ...
          begin
            ...
            //  B.InvFund,
              ...
              sql:='select frompk ,code,itemtablename,itemtablebillcode,dlid,wareid,partno, qty,price,Fund ,warefund,taxamt,TaxRate,note '
              ...
              FhlKnl1.Ds_CopyValues(true,  dmfrm.FreeQuery1 ,  dlDataSet1, CpySourceFlds,CpyDestFlds,true,true) //多选
          end;
          showmodal;
        end;
      end;
...
procedure Twrtransformbillfrm.iniuserBtns;
...
function Twrtransformbillfrm.IsValideBill: boolean;
begin
                   with dlDataSet1 do
                   begin
                         DisableControls;
...
procedure Twrtransformbillfrm.ActInvAdjfnshldinExecute(Sender: TObject);
begin
  FhlUser.ShowEditorFrm('39',null).ShowModal;

end;

procedure Twrtransformbillfrm.ActInvAdjfnshldoutExecute(Sender: TObject);
begin
FhlUser.ShowEditorFrm('40',null).ShowModal;

end;

procedure Twrtransformbillfrm.ActFnShldInExecute(Sender: TObject);
begin
FhlUser.ShowEditorFrm('23',null).ShowModal;

end;

procedure Twrtransformbillfrm.ActFnShldOutExecute(Sender: TObject);
begin
FhlUser.ShowEditorFrm('24',null).ShowModal;
end;

procedure TWrTransformBillFrm.ActIsLockExecute(Sender: TObject);
begin
    inherited;
    ...
    fhlknl1.Kl_GetUserQuery( 'exec pr_LockBill '+quotedstr(self.fbilldict.BillCode) +','+quotedstr('0')  ,false);
...
procedure TWrTransformBillFrm.SetLock(lock:boolean);
begin
