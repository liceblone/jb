...
--2006-7-25在审核标志置0后加上更新成本单价                    
 ...
 if @@rowcount<>1                    
  ...
  --                     where Id in (select WareId from ph_arrivedl where ArriveId=@ArriveId)                    
       ...
       update wr_ware set Price=IsNull((select top 1 ( d.Price/(1+m.TaxRate)) from ph_arrivedl d inner join ph_arrive m on d.ArriveId=m.Code and m.WhInCode is Not null and m.IsChk=1 and d.WareId=wr_ware.Id order by m.ArriveDate Desc,d.dlId desc),cost)    
